...
main
  ...
  .append('a')
  .attr('href', href)
  .text('Monthly global surface air temperature anomalies');
...
d3.select('.viz')
  ...
  .append('rect')
  .attr('fill', d => {
    const isMay = timeParse(d.date).getMonth() === 5;
    if (isMay) {
      return d.value > 0 ? '#A10C03' : '#162A81';
    }
    return d.value > 0 ? '#FF300F' : '#4E98CA';
  })
  .attr('x', d => xScale(timeParse(d.date)))
  .attr('y', d => (d.value > 0 ? yScale(d.value) : yScale(0)))
  .attr('width', width / data.length)
  .attr('height', d =>
    d.value > 0 ? yScale(0) - yScale(d.value) : yScale(d.value) - yScale(0)
  );
