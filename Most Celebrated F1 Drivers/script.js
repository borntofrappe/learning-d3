...
function drawBarChart() {
  ...
  /* INTRODUCTORY MARKUP */
  d3.select("#wrapper").append("h1").text("Most celebrated F1 drivers");
  ...
  d3.select("#wrapper")
    ...
    .html(
      `Highlighting the ${metrics
        ...
        .join("")} number of races awarded to F1 drivers with at least ${
        winsThreshold > 1 ? `${winsThreshold} wins` : `${winsThreshold} win`
      }.`
    );
  ...
  d3.select("#wrapper")
    .append("p")
    .html(
      'Inspired by <a href="https://pudding.cool/2017/08/the-office/">this essay</a> to practice with D3, data binding and transitions.'
    );
  ...
  function drawBars(metric) {
    ...
    */
    ...
    const secondTransition = d3.transition().delay(200).duration(1000);
