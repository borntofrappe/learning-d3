...
main
  .append('p')
  .html(
    'What the average web developer spends time thinking about. 1990-2020<br/>Feel Free to make your own legend.'
  );
...
const randomBeta = ({ alpha = 2.5, beta = 2.5 } = {}) =>
...
const bin = d3
  ...
  .thresholds(scaleValues.ticks(ticks));
...
const dataValues = colors.map((color, index) => {
  ...
  const betas = Array(dataPoints)
    .fill()
    .map(() => randomBeta({ alpha, beta }));
  const bins = bin(betas);
  const values = bins.map(bin => bin.length).slice(0, ticks);
  return values;
});

const dataTotals = Array(ticks)
  .fill()
  .map((value, index) =>
    dataValues.map(values => values[index]).reduce((acc, curr) => acc + curr, 0)
  );
...
const stack = d3.stack().keys(colors);
...
const scaleTime = d3
  .scaleTime()
  .domain([new Date('1990'), new Date('2020')])
  .range([0, ticks - 2]);
...
const xAxis = d3
  .axisBottom(scaleX)
  .ticks(4)
  .tickFormat(d => formatDate(scaleTime.invert(d)))
  .tickSize(0)
  .tickPadding(5);
